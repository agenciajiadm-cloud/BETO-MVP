...
function Boxes({ index, stage }: { index: number; stage: number }) {
      ...
      <>
        <Box on={stage === 0}>
          ...
          <ul className="mt-3 grid grid-cols-2 gap-2 text-sm">
            {sectors.map((s) => (
              <li key={s} className="border border-black/10 bg-branco px-3 py-2">
                {s}
              </li>
            ))}
          </ul>
        ...
        <Box on={stage === 1}>
          ...
          <h3 className="mt-1 text-xl font-semibold md:text-2xl">Galeria Tapera</h3>
      ...
      <>
        <Box on={stage === 0}>
          ...
          <ul className="mt-3 space-y-2 text-sm">
            {formats.map((f) => (
              <li key={f} className="border-b border-black/10 pb-2">
                {f}
              </li>
            ))}
          </ul>
        ...
        <Box on={stage === 1}>
          ...
          <div className="mt-3 space-y-3">
            {services.map((s) => (
              <Link key={s.href} href={s.href} className="block border-b border-black/10 pb-3">
        ...
        <Box on={stage === 2}>
          <h3 className="text-xl font-semibold md:text-2xl">Vamos conversar.</h3>
          <p className="mt-2 text-sm text-mute">Escolha o caminho ou fale direto.</p>
          <a
            href={LINKEDIN}
            target="_blank"
            rel="noreferrer"
            className="mt-4 inline-flex bg-petroleo px-4 py-2 text-sm text-branco"
          >
            Conversar comigo
          </a>
        </Box>
      ...
      <>
        <Box on={stage === 0}>
          <img
            src="/roberto.jpg"
            alt="Roberto Machado"
            className="mb-4 aspect-[16/9] w-full object-cover object-top md:aspect-[4/3]"
          />
          <h3 className="text-xl font-semibold md:text-2xl">Pessoas constroem operações.</h3>
          <p className="mt-2 text-sm text-mute">
        ...
        <Box on={stage === 2}>
          ...
          <h3 className="mt-1 text-xl font-semibold md:text-2xl">Mais que currículo.</h3>
          <p className="mt-2 text-sm text-mute">
  ...
  return (
    <>
      <Box on={stage === 0}>
        <h3 className="text-xl font-semibold md:text-2xl">Tudo conectado. Vamos conversar?</h3>
      ...
      <Box on={stage === 2}>
        <h3 className="text-xl font-semibold md:text-2xl">Vamos olhar sua operação juntos?</h3>
        <a
          href={LINKEDIN}
          target="_blank"
          rel="noreferrer"
...
export function TalkSector({ quadrant, index, stage }: Props) {
  return (
    <div className="container-rm flex h-full max-w-3xl flex-col justify-center px-2 pb-6 pt-16 text-center md:px-0 md:py-6 md:text-left">
      <h2 className="text-[2rem] font-semibold leading-tight tracking-tight md:text-5xl">{quadrant.name}</h2>
      <p className="mx-auto mt-3 max-w-md text-base text-mute md:mx-0 md:text-xl">{quadrant.subtitle}</p>
      <div className="talk-stack mt-6">
